    ...
    <Box>
      <Box pb="3em">
        ...
        <Module
          ...
          bigText
          content={sets}
          logoWidth={responsive("6.428em", "50%")}
        />
      </Box>
      {data.map(
        (
          { title, step, src, subTitle, text, logoWidth, source, content },
          i
        ) => (
          <Box key={i}>
              ...
              {content.map((d, i) =>
                isArray(d) ? (
                  <Box mt={responsive("3em", "2em")} key={i}>
                    {subTitle && (
                      <Text.SmallTitle>{subTitle[i]}</Text.SmallTitle>
                    )}
